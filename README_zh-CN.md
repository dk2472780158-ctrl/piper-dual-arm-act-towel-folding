...
| 试验 | 视频时间戳 | 最终成功 | 人工复位 |
|---|---|---|---|
| 01 – 10 | 见 `results/consecutive_10_trials.csv` | 由录像逐帧回填 | 逐次回填 |

- 连续十次完整版录像（未剪辑、标记 Trial 01–10）：**链接待补**（不入库，见下载一节）。
- 技术分析视频（60–90 秒，阶段标记 + 真实失败案例）：见 `docs/video-script.md`。

**报告纪律：** 我们写 *10/10 consecutive trials*，绝不写「成功率 99%」或「成功率 100%」。单次连续拍摄比聚合百分比更可信。
